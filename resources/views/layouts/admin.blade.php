    ...
    >
        ...
        <nav class="flex-1 overflow-y-auto no-scrollbar py-4 px-3 space-y-1">
            ...
            <a href="{{ route('admin.backorders') }}"
               class="sidebar-nav-item {{ request()->routeIs('admin.backorders*') ? 'active' : '' }}"
               :title="sidebarCollapsed ? 'Back Orders' : ''">
                {{-- Refresh/cycle icon --}}
                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
                </svg>
                <span x-show="!sidebarCollapsed" class="text-sm">Back Orders</span>
                @php $activeBoCount = \App\Models\OrderBackorder::whereIn('status', ['ready', 'dispatched'])->count(); @endphp
                @if($activeBoCount > 0)
                <span x-show="!sidebarCollapsed" class="ml-auto badge badge-gold text-[10px] py-0.5 px-2">{{ $activeBoCount }}</span>
                @endif
            </a>
            ...
            <a href="{{ route('admin.reviews') }}"
               ...
               :title="sidebarCollapsed ? 'Reviews' : ''">
                ...
                <span x-show="!sidebarCollapsed" class="text-sm">Reviews</span>
                @if($pendingReviews > 0)
                <span x-show="!sidebarCollapsed" class="ml-auto badge badge-gold text-[10px] py-0.5 px-2">{{ $pendingReviews }}</span>
                @endif
            </a>
            ...
            <a href="{{ route('admin.finance') }}"
               class="sidebar-nav-item {{ request()->routeIs('admin.finance*') ? 'active' : '' }}"
               :title="sidebarCollapsed ? 'Finance' : ''">
                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                </svg>
                <span x-show="!sidebarCollapsed" class="text-sm">Finance</span>
            </a>
        </nav>
