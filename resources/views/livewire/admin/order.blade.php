...
>
                    ...
                    <tr wire:key="order-{{ $order->id }}" class="{{ $pendingReceipt ? 'bg-amber-50/30' : '' }}">
                        ...
                        <td>
                            <div class="flex items-center gap-1.5 flex-wrap">
                                ...
                                {{-- Confirm Order --}}
                                @if($order->status === 'payment_received' || ($order->status === 'new' && $order->payment_method !== 'bank_transfer'))
    ...
    >
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div>
                ...
                @if($summaryBalanceDue > 0 && !in_array($selectedOrder->status, ['cancelled', 'refunded', 'completed']))
                <div class="mt-3">
                    <button wire:click="openBalancePayModal({{ $selectedOrder->id }})"
                            class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-700 text-xs font-semibold hover:bg-emerald-100 transition-all">
    ...
    ══════════════════════════════════════════════════════════════ --}}
    @if($showBalancePayModal)
    <div class="fixed inset-0 bg-slate-950/50 backdrop-blur-sm z-50"></div>
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                <h3 class="font-[Poppins] font-bold text-[#0F172A] text-base">Record Balance Payment</h3>
                <button wire:click="$set('showBalancePayModal', false)" class="p-2 rounded-xl text-slate-400 hover:bg-slate-100 transition-all">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                    </svg>
                </button>
            ...
            <div class="px-6 py-4 space-y-4">
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1.5">Amount (Rs.) <span class="text-red-500">*</span></label>
                    <input type="number" wire:model="balancePayAmount" step="0.01" min="1"
                           class="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-amber-300 focus:border-amber-400 outline-none"/>
                    @error('balancePayAmount') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1.5">Payment Method <span class="text-red-500">*</span></label>
                    <select wire:model="balancePayMethod" class="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-amber-300 outline-none">
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1.5">Reference / Transaction ID</label>
                    <input type="text" wire:model="balancePayReference" placeholder="Optional"
                           class="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-amber-300 outline-none"/>
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1.5">Notes</label>
                    <textarea wire:model="balancePayNotes" rows="2" placeholder="Optional"
                              class="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-amber-300 outline-none resize-none"></textarea>
                </div>
            </div>
            <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
                <button wire:click="$set('showBalancePayModal', false)"
                ...
                <button wire:click="confirmBalancePayment" wire:loading.attr="disabled" wire:target="confirmBalancePayment"
                        class="px-4 py-2 rounded-xl bg-emerald-500 text-white text-sm font-semibold hover:bg-emerald-600 transition-all">
                    <span wire:loading.remove wire:target="confirmBalancePayment">Confirm Payment</span>
                    <span wire:loading wire:target="confirmBalancePayment">Saving...</span>
                </button>
            </div>
        </div>
    </div>
